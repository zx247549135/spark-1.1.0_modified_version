...
private[spark] class MapOutputTrackerMaster(conf: SparkConf)
  ...
  override def unregisterShuffle(shuffleId: Int) {
    ...
    if(reducePreferredLocs.contains(shuffleId))
      reducePreferredLocs.remove(shuffleId)
  ...
  private def cleanup(cleanupTime: Long) {
    ...
    reducePreferredLocs.clearOldValues(cleanupTime)
    cachedSerializedStatuses.clearOldValues(cleanupTime)
  }

  private def getPreferredLocsByMapStatuses(shuffleId: Int): Array[String] ={
    ...
    statuses.map {
      status =>
        if (status == null) {
          ...
          (status.location,status.fileGroupID, decompressSize(status.compressedSizes(reduceId)))
        }
    }
  }

  def convertPreferredLocsByMapStatuses(
       shuffleId: Int,
       statuses: Array[MapStatus]): Array[String] = {
    assert (statuses != null)
    ...
    statuses.map {
      status =>
        if (status == null) {
          ...
          numReduces=status.compressedSizes.length
          ...
          var i=0
          while(i<numReduces){
            val byteSize=decompressSize(status.compressedSizes(i))
    ...
    if(splitsByHost.size>0) {
      ...
      val splitSumOfByteSizeOfHost=new Array[Long](numOfHosts)

      for(i <- 0 until sortedIndexOfBytesOfHost.length){
        var minIndex=0
        for(j <- 1 until splitSumOfByteSizeOfHost.length){
            if(splitSumOfByteSizeOfHost(j)<splitSumOfByteSizeOfHost(minIndex)){
              minIndex=j
            }
        }
        splitSumOfByteSizeOfHost(minIndex)+=sortedIndexOfBytesOfHost(i)._2
