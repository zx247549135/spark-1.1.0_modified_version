...
class ShuffleBlockManager(blockManager: BlockManager,
    ...
    new ShuffleWriterGroup {
      ...
      private def newFileGroup(): ShuffleFileGroup = {
        ...
        val writers: Array[BlockObjectWriter] =Array.tabulate[BlockObjectWriter](numBuckets) { bucketId =>
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)
          blockManager.getDiskWriter(blockId, files(bucketId), serializer, bufferSize,
            writeMetrics)
        }
  ...
  def closeShuffleWriters(shuffleId:Int) {
    // Search all file groups associated with this shuffle.
    val shuffleState = shuffleStates(shuffleId)
...
object ShuffleBlockManager {
   ...
   */
  private class ShuffleFileGroup(val shuffleId: Int, val fileId: Int, val files: Array[File],val writers: Array[BlockObjectWriter]) {
   ...
   */
     def getFileSegmentFor(mapId: Int, reducerId: Int): Option[FileSegment] = {
      ...
      /*   var sumOflength:Long=0
         var i=0
         while(i<blockLengthsByReducer(reducerId).length){
           sumOflength+=blockLengthsByReducer(reducerId)(i)
           i+=1
         }
         Some(new FileSegment(file, 0,sumOflength)) */
       } else {
         None
       }
     }
